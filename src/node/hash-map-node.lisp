            ...
            (let* ((current (at-position dmap position))
              ...
              (if (equal key current-key)
                  ...
                  (let ((new-node (-> (empty-node node context)
                                    (put current (list (h:hash current-key) (1+ depth) bit-size))
                                    (put entry    (list hash (1+ depth) bit-size)))))
                    (-> node
                      (remove position)
                      (insert position new-node)))))
            ;; no data, so do we have a node already then for this depth
            (if (is-set nmap position)
                (update node position (put (at-position nmap position) entry (list hash (1+ depth) bit-size)))
        ...
        (if (is-set dmap position)
            (let ((target (at-position dmap position)))
              (if (equal key (e:key target))
                  ...
                  default))
            (if (is-set nmap position)
                (get (at-position nmap position) key (list hash (1+ depth) bit-size default))
                default))))))
        ...
        (if (is-set dmap position)
            ...
            (when (is-set nmap position)
              (let* ((sub-node (at-position nmap position))
                     (new-node (del sub-node key (list hash (1+ depth) bit-size))))
