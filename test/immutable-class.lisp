...
(test :using-setf-to-change-slot-values
  ...
  (let ((my-foo (make-instance 'foo :data 1)))
    (is (= 1 (:data my-foo)))
    (signals p::invalid-access-to-immutable-object (setf (:data my-foo) 2))
    (is (= 1 (:data my-foo)))
    (signals p::invalid-access-to-immutable-object (setf (slot-value my-foo 'data) 3))
    (is (= 1 (:data my-foo)))
    (with-slots (data) my-foo
      (signals p::invalid-access-to-immutable-object (setf data 4)))
    (is (= 1 (:data my-foo)))))
...
(p::define-immutable-class bar (foo a-mixin)
...
(test :mixins-work-as-expected
  :description "using a mixin, default to immutable when mixed into a mutable class; remains mutable otherwise"
  (let ((my-bar (make-instance 'bar :data 1 :other 1))
        (my-baz (make-instance 'baz :other 2)))
    (setf (:other my-baz) 10)
    (is (= 10 (mixin-fn my-baz)))
    (signals p::invalid-access-to-immutable-object (setf (:other my-bar) 10))))
